"""Isometric tile generator (Phase 1).

Builds clean 2:1 isometric tiles by snapping AI-supplied *texture* onto exact
geometric masks, so edges are pixel-perfect and every tile shares one grid:

  - project a flat top-down texture onto the ideal diamond top (2:1),
  - extrude the diamond straight down for the cube side faces (shaded L/R for AO),
  - derive height variants (full / half / quarter / slab) by changing only the
    extrusion depth, so all variants share the identical diamond top,
  - quantize to a retro palette for clean colours.

Exports (for Godot 4 and others):
  - individual tile PNGs,
  - a uniform-grid atlas PNG (cells W x cell_h, tiles bottom-centre anchored),
  - a Godot 4 ``.tres`` TileSet (isometric, tile_size = W x W/2),
  - an import-notes text file.
"""
...
# Valid 2:1 tile widths (diamond top is W wide, W//2 tall).
VALID_ISO_WIDTHS = (32, 64, 128)
DEFAULT_ISO_WIDTH = 64

# Height variants: name -> fraction of the full side depth (full depth = W//2).
HEIGHT_VARIANTS = ("full", "half", "quarter", "slab")
_DEPTH_FRACTION = {"full": 1.0, "half": 0.5, "quarter": 0.25, "slab": 0.0}

OUTLINE = (24, 28, 26, 255)
_LEFT_SHADE = 0.55   # left face darkness (shadow side)
_RIGHT_SHADE = 0.72  # right face darkness (lit side)
_RIM_SHADE = 0.82    # top material wrapping a few px down over the side ("lip")
...
def project_top(texture: np.ndarray, width: int) -> Tuple[np.ndarray, List[int]]:
    """Project a flat square texture onto the iso top diamond.

    Returns (top_rgba of shape (htop, width, 4), bottom_row per column) where
    bottom_row[x] is the lowest opaque row of the diamond at column x (-1 if the
    column is empty), used to extrude the side faces.
    """
    htop = width // 2
    ...
    for y in range(htop):
        for x in range(width):
            if not _in_diamond(x, y, cx, cyc, width, htop):
                continue
            # Inverse isometric map -> surface coords (u, v) in [0, 1].
            sx = (x - cx) / (width / 2.0)
...
def _representative_color(top: np.ndarray) -> Tuple[int, int, int]:
    """Median colour of the opaque top pixels (robust base for side shading)."""
    mask = top[:, :, 3] > 0
    px = top[:, :, :3][mask]
    if px.size == 0:
        return (120, 120, 120)
...
def _rim_height(width: int, depth: int) -> int:
    """How many pixels the top material wraps down over the side faces.

    A few px, scaled to tile size, but always leaving at least 1px of side
    material below it (so short tiles still read as a block, not all-grass).
    """
    rim = max(2, round(width / 20))      # ~3px at 64-wide
...
def build_tile(top: np.ndarray, bottom_row: List[int], width: int, depth: int,
    ...
    """Compose one iso tile (top + extruded sides + outline) bottom-anchored.

    When ``rim`` is true, the top material wraps a few pixels down over the side
    faces (following the diamond edge) before the side material takes over.
    """
    htop = width // 2
    H = htop + depth + 1
    ...
    for x in range(width):
        ...
        # Per-column rim colour sampled from the top texture at the diamond edge,
        # shaded for the vertical face -> the grass "lip" follows the edge shape.
...
def _quantize_texture(texture: np.ndarray, palette: str, colors: int) -> np.ndarray:
    """Quantize the source texture once so all tiles share one clean palette."""
...
def _build_atlas(tiles: Dict[str, np.ndarray], width: int) -> Tuple[bytes, int, List[str], int]:
    """Pack tiles into a uniform horizontal grid, bottom-centre anchored.

    Returns (atlas_png, cell_h, ordered_names, n_cols).
    """
    ...
    for i, name in enumerate(names):
        t = tiles[name]
        th = t.shape[0]
        atlas.paste(Image.fromarray(t, "RGBA"), (i * width, cell_h - th))  # bottom-anchored
    buf = io.BytesIO()
...
def _godot_tres(width: int, cell_h: int, names: List[str], atlas_name: str,
                folder: str) -> str:
    """Generate a Godot 4 TileSet resource for the atlas.

    tile_shape = 1 (isometric), tile_size = (W, W/2). Each variant is one atlas
    tile; texture_origin shifts the (taller) region so the diamond base sits on
    the cell. The atlas path assumes the export folder sits at the project root
    (res://<folder>/), which is the default when you drop the unzipped folder in.
    """
    htop = width // 2
    lines = [
    ...
    # Default draw centres the region on the cell; shift down so the diamond base
    # (bottom of the cell content) aligns to the iso cell. origin.y moves the
    # region up by half the surplus height above the diamond base.
...
def make_isometric_tiles(texture_bytes: bytes, width: int = DEFAULT_ISO_WIDTH,
    ...
    """Build an isometric tileset from a flat top texture.

    Returns a dict with the per-variant tile PNGs, a packed atlas PNG, a Godot
    ``.tres`` TileSet, import notes, and a large preview PNG.
    """
    ...
    folder = basename  # the folder name inside the export zip / in the project
    ...
    # One-click export: a zip with everything inside a single named folder.
    zbuf = io.BytesIO()
    ...
    # Large preview: the variants in a row on a neutral backdrop.
